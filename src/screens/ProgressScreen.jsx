...
const SUBJECT_MASTERY = [
  { name: 'Physics',     icon: '⚡', percent: 72, from: 'from-blue-500',    to: 'to-indigo-600' },
  { name: 'Chemistry',   icon: '🧪', percent: 55, from: 'from-orange-400',  to: 'to-amber-600' },
  { name: 'Maths',       icon: '📐', percent: 88, from: 'from-violet-500',  to: 'to-purple-700' },
...
export default function ProgressScreen({ onNavigate }) {
  ...
  return (
    <div className="min-h-screen bg-brand-bg pb-40 md:pb-8">
      ...
      {/* Identity card */}
      <div className="bg-gradient-to-br from-brand-dark via-brand-primary to-violet-600 px-5 pt-12 md:pt-6 pb-6">
        <div className="flex items-center gap-4">
          {/* Avatar */}
          <div className="w-16 h-16 rounded-2xl bg-white/20 flex items-center justify-center text-white text-xl font-black border-2 border-white/30">
            {student.avatar}
          </div>
          <div className="flex-1">
            <h1 className="text-white font-black text-xl leading-tight">{student.name}</h1>
            <p className="text-white/60 text-xs mt-0.5">Class {student.class} · {student.school.split(',')[0]}</p>
            <div className="flex items-center gap-2 mt-1.5">
              <span className="text-xs font-black text-amber-300 bg-white/10 px-2.5 py-0.5 rounded-full">Lv {student.level} — {student.levelName}</span>
            </div>
          </div>
          <div className="relative flex-shrink-0">
            <ScoreRing percent={xpPercent} size={52} strokeWidth={5} />
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-[10px] font-black text-white">{student.xp}</span>
              <span className="text-[8px] text-white/60">XP</span>
            </div>
          </div>
        </div>

        <div className="mt-4 h-1.5 bg-white/20 rounded-full overflow-hidden">
          <div className="h-full bg-amber-300 rounded-full" style={{ width: `${xpPercent}%` }} />
        </div>
        <p className="text-white/50 text-[10px] mt-1">{xpToNext - xpInLevel} XP to Level {student.level + 1}</p>
      </div>
      ...
      <div className="px-4 space-y-4 mt-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <div key={stat.label} className="bg-white rounded-2xl p-3 shadow-card text-center">
          ...
          <div className="flex gap-3 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
            {earnedBadges.map(badge => (
              <div key={badge.id} className="flex-shrink-0 flex flex-col items-center gap-1.5 w-16">
                <div className="w-14 h-14 bg-amber-50 border-2 border-amber-200 rounded-2xl flex items-center justify-center text-2xl shadow-sm">
                  {badge.icon}
                </div>
            ...
            {lockedBadges.slice(0, 2).map(badge => (
              <div key={badge.id} className="flex-shrink-0 flex flex-col items-center gap-1.5 w-16 opacity-40">
                <div className="w-14 h-14 bg-gray-100 border-2 border-gray-200 rounded-2xl flex items-center justify-center text-2xl grayscale">
                  {badge.icon}
                </div>
        ...
        <div className="bg-white rounded-3xl p-4 shadow-card">
          ...
          <button
            ...
            className="w-full bg-brand-primary/10 text-brand-primary font-bold text-sm py-2.5 rounded-2xl active:scale-95 transition-all"
          >
            Fix Physics gap →
          </button>
        </div>
        ...
        <div className="bg-white rounded-3xl p-4 shadow-card">
          ...
          <div className="space-y-2">
            {levels.map(lvl => {
              ...
              return (
                <div key={lvl.level} className={`flex items-center gap-3 px-3 py-3 rounded-2xl ${
                  isCurrent ? 'bg-gradient-to-r from-brand-primary to-violet-600' :
                  ...
                  <div className="flex-1">
                    ...
                    <p className={`text-xs ${isCurrent ? 'text-violet-200' : 'text-brand-slate'}`}>{lvl.xpRequired.toLocaleString()} XP</p>
                  </div>
        ...
        <div className="flex gap-3">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 bg-white rounded-2xl py-3 shadow-card text-xs font-semibold text-brand-slate"
          >
